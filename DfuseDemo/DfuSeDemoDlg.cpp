...
BEGIN_MESSAGE_MAP(CDfuSeDemoDlg, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_WM_DEVICECHANGE()
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_PROGRESS, &CDfuSeDemoDlg::OnNMCustomdrawProgress)
	ON_BN_CLICKED(IDC_CHECKUPGRADEOPTIMIZE, &CDfuSeDemoDlg::OnBnClickedCheckupgradeoptimize)
	ON_LBN_SELCHANGE(IDC_LISTFILETARGETS, &CDfuSeDemoDlg::OnLbnSelchangeListfiletargets)
END_MESSAGE_MAP()
...
BOOL CDfuSeDemoDlg::OnInitDialog()
{
	...
	MoveWindow(rc.left, rc.top, rc.Width(), rc.Height()/2);
	//m_Progress.GetWindowRect(&rc1);
	//m_Progress.MoveWindow(rc.left, (rc.Height()-rc1.Height())/2, rc.Width(), rc.Height() / 2)
...
//=========================================================================================================================
						...
						if((SupportROP) && ( STM32Serie == 3))   // STM32L1
						{
							for (DWORD j=0;j<m_pMapping[i].NbSectors;j++)
							{
								if (RDP == 0xAA)
								...
								if (i == 0)  // Write protection apply only on Internal Flash , alternate 0 //
								{
									...
									else if (j < 24)  WBIT =  (WRP2 >> (j-16))  & 0x01;
									else if (j < 32)  WBIT =  (WRP3 >> (j-24))  & 0x01;
									else if (j < 40)  WBIT =  (WRP4 >> (j-32))  & 0x01;
									else if (j < 48)  WBIT =  (WRP5 >> (j-40))  & 0x01;
									else if (j < 56)  WBIT =  (WRP6 >> (j-48))  & 0x01;
									else if (j < 64)  WBIT =  (WRP7 >> (j-56))  & 0x01;
									else if (j < 72)  WBIT =  (WRP8 >> (j-64))  & 0x01;
									else if (j < 80)  WBIT =  (WRP9 >> (j-72))  & 0x01;
...
void CDfuSeDemoDlg::TakeMCUOutofDFUMode()
{
	DFUThreadContext Context;
	CString Tempo;
	DWORD dwRet;
		...
		else
		{
			...
			case OPERATION_UPGRADE:
				...
				//Tmp.Format("Target %02i: Upgrading - Download Phase (%i%%)...", m_CurrentTarget, Context.Percent);
				  Tmp.Format("Upgrading - Download Phase (%i%%)...", Context.Percent);
			...
			default:
				...
				Tmp.Format("Upgrading - Erase Phase (%i%%)...", Context.Percent);
				break;
			}
...
void CDfuSeDemoDlg::OnDblclkListtargets(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (Sel!=-1)
	{
		...
		if (MapName.Compare("Option Bytes  ") == 0)
		{
			...
			if((!ReadProtected) && (STM32Serie == 3))
			{
				COptionBytesDialog_L1 OpbDlg;
				if (STDFU_Open((LPSTR)(LPCSTR)m_CurrDFUName,&OpbDlg.m_phDevice)==STDFU_NOERROR)
				{
					OpbDlg.DoModal();
				}
...
void CDfuSeDemoDlg::OnNMCustomdrawProgress(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CDfuSeDemoDlg::OnBnClickedCheckupgradeoptimize()
{
	// TODO: Add your control notification handler code here
}


void CDfuSeDemoDlg::OnLbnSelchangeListfiletargets()
{
	// TODO: Add your control notification handler code here
}
